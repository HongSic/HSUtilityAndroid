package hs.utility.android.Badge;
...
import hs.utility.android.Badge.implement.*;
...
//        BADGERS.add(LGHomeBadger.class);
    ...
    private static boolean initBadger(Context context) {
        ...
        if (sShortcutBadger == null||sShortcutBadger.equals(""))
        {
            if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi"))
                sShortcutBadger = new XiaomiHomeBadger();
            else sShortcutBadger = new DefaultBadger();
        }
        //삼성폰은 킷캣이상부터 기본 인텐트을 쓴다 (구글링 모띠~)
        //Samsung phones use basic intent from KitKat or more
        else if(sShortcutBadger.getClass().toString().equals(SamsungHomeBadger.class.toString())&&
                Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT)sShortcutBadger = new DefaultBadger();
